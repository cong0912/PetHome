...
const AccountList = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await MyAxios.get("api/v1/user");
  ...
  const handleConfirmStatusChange = async () => {
    try {
      ...
      await MyAxios.get(`api/v1/user/disable?id=${selectedRow.id}`, { isDisabled: updatedUser.isDisabled });
      setRows(rows.map((row) => (row.id === selectedRow.id ? updatedUser : row)));
    } catch (error) {
